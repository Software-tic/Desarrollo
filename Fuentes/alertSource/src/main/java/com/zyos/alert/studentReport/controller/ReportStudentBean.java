...
@ManagedBean
@ViewScoped
@URLMappings(mappings = {@URLMapping(id = "reportStudent", pattern = "/portal/reportes", viewId = "/pages/studentReport/studentReport.jspx"),
	@URLMapping(id = "studentRisk", pattern = "/portal/estudiantesRiesgo", viewId = "/pages/riskStudent/riskStudent.jspx")})
public class ReportStudentBean extends ZyosBackingBean {
	...
	// Render to report graphic
	private boolean showGraphic, showReport, showRisk, showActiveStudent, showManual, showRiskDegree, showRiskFaculty;

	// Render to student risk
	...
	private ReportStudentController controller = new ReportStudentController();

	public ReportStudentBean() throws Exception {
		try {
			String contextPath = PrettyContext.getCurrentInstance().getCurrentMapping().getId();
			if (contextPath.equals("studentRisk")) {
				if (reportStudentList == null) {
					reportSearch = new ReportStudent();
					studentDataSelected = new Student();
					degreeList = controller.loadDegreeList();
	...
	public void loadReportSearchList() {
		try {
			...
			reportFacultyStudentList = controller.loadReportFacultyStudentList();
			reportRiskFactorCategoryList = controller.loadReportRiskFactorCategoryList();
	...
	public void loadRiskFactorReportStudent() {
		try {
			if (reportSearch.getIdRiskFactorCategory() != null && !reportSearch.getIdRiskFactorCategory().equals(0L)) {
				reporStudentRiskFactorList = controller.loadReporStudentRiskFactorList(reportSearch.getIdRiskFactorCategory());
			}
	...
	public void loadReportStudentList() {
		try {
			reportStudentList = controller.loadReportStudentList(getUserSession().getDefaultGroup(), getUserSession().getId(), reportSearch);
		} catch (Exception e) {
			ErrorNotificacion.handleErrorMailNotification(e, this);
		}
	}
	...
	public void searchReportStudentList() {
		try {
			if (reportSearch != null) {
				if (dateFrom != null) {
					reportSearch.setDateFrom(ManageDate.formatDate(dateFrom, ManageDate.YYYY_MM_DD_HH_MM_SS));
				}
				if (dateTo != null) {
					reportSearch.setDateTo(ManageDate.formatDate(dateTo, ManageDate.YYYY_MM_DD_HH_MM_SS));
				}
				dateFrom = null;
				dateTo = null;
				if (validateSearch()) {
					reportStudentList =
						controller.loadSearchReportStudentList(getUserSession().getDefaultGroup(), getUserSession().getId(), reportSearch);
	...
	public boolean validateSearch() {
		try {
			if (reportSearch != null) {
				if (reportSearch.getDateFrom() == null && reportSearch.getDateTo() == null && reportSearch.getIdSolicitor().equals(0L)
					&& reportSearch.getIdStatusReportStudent().equals(0L) && reportSearch.getIdFaculty().equals(0L)
					&& reportSearch.getIdReportType().equals(0L) && reportSearch.getIdRiskFactorCategory().equals(0L)
					&& (reportSearch.getCode() == null || reportSearch.getCode().isEmpty())) {
					return false;
				}
				return true;
			} else {
	...
	public void generateReportStudentList() {
		try {
			...
			if (reportStudentList != null && reportStudentList.size() > 0) {
				...
				for (ReportStudent c : reportStudentList) {
					...
					for (int k = 0; k <= 14; k++) {
						...
						if (k == 2) { // name
							...
							if (c.getZyosUserName() != null && c.getZyosUserLastName() != null) {
								cell.setCellValue(c.getZyosUserName() + " " + c.getZyosUserLastName());
							} else {
								if (c.getZyosUserName() != null && c.getZyosUserLastName() == null) {
									...
									if (c.getZyosUserName() == null && c.getZyosUserLastName() != null) {
										cell.setCellValue(c.getZyosUserLastName());
									}
								}
							}
						...
						if (k == 4) {
							...
							cell.setCellValue(c.getDocumentNumber() != null ? c.getDocumentNumber() : " ");
						}
						if (k == 5) {
							cell.setCellValue(c.getPhone() != null ? c.getPhone() : " ");
						}
						if (k == 6) {
							cell.setCellValue(c.getMobilePhone() != null ? c.getMobilePhone() : " ");
						}
						if (k == 7) {
							cell.setCellValue(c.getEmailStudent() != null ? c.getEmailStudent() : " ");
						}
						if (k == 8) {

							cell.setCellValue(c.getIdReportType() != null && !c.getIdReportType().equals(0L) ? getNameLabelList(
								getReportStudentReportType(), c.getIdReportType()) : " ");
						}
						if (k == 9) {
							cell.setCellValue(c.getDateCreation() != null ? c.getDateCreation() : " ");
						}
						if (k == 10) {
							cell.setCellValue(c.getIdReportType() != null && !c.getIdRiskFactorCategory().equals(0L) ? getNameLabelList(
								getReportRiskFactorCategoryList(), c.getIdRiskFactorCategory()) : " ");
						}
						if (k == 11) {
							cell.setCellValue(c.getRiskFactorName() != null ? c.getRiskFactorName() : " ");
						}
						if (k == 12) {

							String nameSolicitor = null;

							if (c.getIdReportType().equals(IReportType.AUTOMATIC)) {
						...
						if (k == 13) {
							...
							cell.setCellValue(!c.getIdStatusReportStudent().equals(null) && !c.getIdStatusReportStudent().equals(0L) ? getNameLabelList(
								getReportStatusList(), c.getIdStatusReportStudent()) : " ");
						}
				...
				ze.closeFile(wb, file.getAbsolutePath() + "/reporteEstudiantes.xlsx");
			} else {
				ZyosBackingBean.addWarn("Generar reporte", " No cuenta con estudiantes reportados en el listado para generar reporte");
	...
	public void handleDataExporter(Object o) {
		try {
			...
			labelTableList.add(ZyosBackingBean.getMessage("page.riskStudentBean.labelDate"));
			labelTableList.add(ZyosBackingBean.getMessage("page.riskStudentBean.labelDays"));
			labelTableList.add(ZyosBackingBean.getMessage("page.riskStudentBean.labelRole"));
			labelTableList.add(ZyosBackingBean.getMessage("page.riskStudentBean.labelResponsible"));
			labelTableList.add(ZyosBackingBean.getMessage("page.riskStudentBean.labelObservation"));
	...
	/** jhernandez */
	/*
	 * public void validateStagePermission() { try { for (Stage s : getStagePermissionList()) { if
	 * (getUserSession().getDefaultGroup().equals(s.getIdStage())) { showButtonGiveProcess = true; }
	 * } } catch (Exception e) { ErrorNotificacion.handleErrorMailNotification(e, this); } }
	 */
		...
		try {
			...
			switch (graphicToShow.intValue()) {
				case 0:
					...
					chartSeriesReport = "[['Manualmente'," + obj[0] + "],['Automaticamente'," + obj[1] + "]]";
				...
				case 1:
					...
					resultList.add(0, controller.loadReportStudentManualDataClassMate());
					resultList.add(1, controller.loadReportStudentManualDataFreeUser());
					resultList.add(2, controller.loadReportStudentManualDataTeacher());
				...
				case 2:
					if (detailRiskFactor) {
						...
						HashMap<String, Float> datailPieData = controller.loadDetailRiskFactor(idRiskFactorCategorySelected);
						int count = 0;
						for (String key : datailPieData.keySet()) {
							chartSeriesReport += "['" + key + "'," + datailPieData.get(key) + "]";
							count++;
						...
						// chartSeriesReport = "[";
						series = new JSONArray();
						List<GraphicData> tmpPieData = controller.loadStudentByRiskData();

						chartSeriesReport =
							"[['Academico'," + tmpPieData.get(0).getAcademicReports() + "],['Socioeconomico',"
								+ tmpPieData.get(0).getSocioeconomicReports() + "]" + " ,['Institucionales',"
								+ tmpPieData.get(0).getInstitutionalReports() + "]" + ",['Personales'," + tmpPieData.get(0).getPersonalReports()
								+ "]]";
						// getRequest().setAttribute("generalReport", chartSeriesReport);
				...
				case 4:
					if (detailRiskFactorDegree) {
						chartSeriesReport = "[";
						series = new JSONArray();
						HashMap<String, Float> studentReportDegree = controller.loadDetailStudentReportDegree(idRiskFactorCategoryDegree);
						degreeStudentList = controller.loadStudentReportDetailDegree(idRiskFactorCategoryDegree);
						detailRiskDegree = true;
						int count = 0;
						for (String key : studentReportDegree.keySet()) {
							chartSeriesReport += "['" + key + "'," + studentReportDegree.get(key) + "]";
							count++;
						...
						chartSeriesReport = "[";
						series = new JSONArray();
						HashMap<String, Float> studentReportDegree = controller.loadStudentReportDegree();
						int count = 0;
						for (String key : studentReportDegree.keySet()) {
							chartSeriesReport += "['" + key + "'," + studentReportDegree.get(key) + "]";
							count++;
				...
				case 5:
					...
					if (detailRiskFactorFaculty) {
						chartSeriesReport = "[";
						series = new JSONArray();
						HashMap<String, Float> studentReportFaculty = controller.loadDetailStudentReportFaculty(idRiskFactorCategoryFaculty);
						facultyStudentList = controller.loadStudentReportDetailFaculty(idRiskFactorCategoryFaculty);

						int count = 0;
						for (String key : studentReportFaculty.keySet()) {
							chartSeriesReport += "['" + key + "'," + studentReportFaculty.get(key) + "]";
							count++;
						...
						chartSeriesReport = "[";
						series = new JSONArray();
						HashMap<String, Float> studentReportFaculty = controller.loadDetailStudentReportFaculty();
						int count = 0;
						for (String key : studentReportFaculty.keySet()) {
							chartSeriesReport += "['" + key + "'," + studentReportFaculty.get(key) + "]";
							count++;
	...
	public void graphDataDetailDegree() {
		try {
			...
			chartSeriesReport = "[";
			series = new JSONArray();
			HashMap<String, Float> studentReportDegree = controller.loadDetailDegreeList(idRiskFactorCategoryDegree, idDegreeSelected);
			int count = 0;
			for (String key : studentReportDegree.keySet()) {
				chartSeriesReport += "['" + key + "'," + studentReportDegree.get(key) + "]";
				count++;
	...
	public void graphDataDetailFaculty() {
		try {
			...
			chartSeriesReport = "[";
			series = new JSONArray();
			HashMap<String, Float> studentReportFaculty = controller.loadDetailFacultyList(idRiskFactorCategoryFaculty, idFacultySelected);
			int count = 0;
			for (String key : studentReportFaculty.keySet()) {
				chartSeriesReport += "['" + key + "'," + studentReportFaculty.get(key) + "]";
				count++;
	...
	public void goDeleteReportStudentDialog() {
		try {
			ZyosBackingBean.getRequestContext().execute("delReportStudentWV.show();");
		} catch (Exception e) {
			ErrorNotificacion.handleErrorMailNotification(e, this);
		}
	...
	public void deleteReportStudent() {
		try {
			if (reportStudentSelected == null) {
				...
				ZyosBackingBean.addWarn("Eliminar reporte", " Error al intentar eliminar reporte seleccionado");

			} else {

				controller.deleteReportStudent(reportStudentSelected, getUserSession().getDocumentNumber());

				ZyosBackingBean.addInfo("Eliminar Reporte", "Reporte eliminado con éxito!");
	...
	public void goDetailStudentReport() {
		try {
			...
			ZyosBackingBean.getRequestContext().execute("updateDataStudentDialog.show();");
	...
	public void goReportStudentList() {
		try {
			showStudentList = true;
			setPanelView("riskStudentList", "Lista Factores Riesgo", "ReportStudentBean");
		} catch (Exception e) {
			ErrorNotificacion.handleErrorMailNotification(e, this);
		}
	}
	...
	public void giveProcessToReportStudent() {
		try {
			...
			loadReportStudentList();
			showStudentList = true;
			addInfo("Dar trámite", "Se ha creado una nueva observación al proceso");
			if (reportStudentSelected.getIdSolicitor() == null && getUserSession().getDefaultGroup().equals(IZyosGroup.ADMINISTRATOR)
				&& reportStudentSelected.getIdStatusReportStudent().equals(IStatusReportStudent.START_INTERVENTION)) {
				threadMailToFamily();
			}
			/*
			 ...
			 * ().equals(IStatusReportStudent.START_INTERVENTION)) threadMailAdviserNotification();
			 */
	...
	/** @author MTorres 19/06/2014 16:08:20 */
	private void sendEmailAdviserNotification(final ReportStudent rs, final ZyosUser a) throws Exception {
		try {
			...
			e.sendProcessEmail(
				null,
				t.getSubject(),
				MailGeneratorFunction.createGenericMessage(t.getBody(), t.getAnalyticsCode(), a.getName() + " " + a.getLastName(),
	...
	public void sendEmailToFamilyStudent(final ReportStudent rs) {
		try {
			EmailTemplate t = MailGeneratorFunction.getEmailTemplate(IEmailTemplate.REGISTER_REPORTSTUDENT, Long.valueOf(1));
			if (t != null) {
				SMTPEmail e = new SMTPEmail();
				e.sendProcessEmail(null, t.getSubject(),
					MailGeneratorFunction.createGenericMessage(t.getBody(), t.getAnalyticsCode(), rs.getZyosUserName(), rs.getZyosUserLastName()),
					rs.getEmailSolicitor());
			} else {
				ZyosBackingBean.addWarn("Alerta", "Error al enviar correo eléctronico al usuario externo. ");
			}
		} catch (Exception e) {
	...
	public void setReportStudentListFiltered(List<ReportStudent> reportStudentListFiltered) {
	...
	public void setIdRiskFactorCategorySelected(Long idRiskFactorCategorySelected) {
	...
	public void setReportFacultyStudentList(List<Faculty> reportFacultyStudentList) {
	...
	public void setReportStudentReportType(List<ReportType> reportStudentReportType) {
	...
	public void setReporStudentRiskFactorList(List<RiskFactor> reporStudentRiskFactorList) {
	...
	public void setReportRiskFactorCategoryList(List<RiskFactorCategory> reportRiskFactorCategoryList) {
	...
	public StreamedContent getErrorFileReportStudent() {
		try {
			...
			File file = new File(".");
			File f = new File(file.getAbsolutePath() + "/reporteEstudiantes.xlsx");
			InputStream ip = new FileInputStream(f);
			errorFileReportStudent = new DefaultStreamedContent(ip, "/reporteEstudiantes", "reporteEstudiantes.xlsx");
